...
//! \ingroup fir
template <class Numeric> class fir_multi_interp : public fir<Numeric>
{
	public: 
	long num_low;
	long channels; //! number of channels to process
...
//! Constructor
    fir_multi_interp<Numeric>(void) : phase(0), auto_mode(1), channels(1) {	};
	fir_multi_interp<Numeric>(long n, long m) : phase(0), auto_mode(1), channels(m)
			{
			int i;
			int mn = m*n;
			fir<Numeric>::num_taps = n;
			if (mn>0) {
				fir<Numeric>::coeff = new Numeric[n];	
				fir<Numeric>::z = new Numeric[mn];
				for (i=0;i<n;i++) fir<Numeric>::coeff[i] = 0;  
				for (i=0;i<mn;i++) fir<Numeric>::z[i] = 0;  
			}
	};
	void reset() {
		fir<Numeric>::reset();
    ...
    void input(Numeric in) {
		...
		for (i=channels*num_low;i>0;i--) fir<Numeric>::z[i] = fir<Numeric>::z[i-1];  
		// Add new input
		fir<Numeric>::z[0] = in;   
	}
	...
	Numeric clock(void) {
		...
		for (output=0,i=0;i<num_low;i++) {
			output += fir<Numeric>::coeff[(i+1)*rate+phase]*fir<Numeric>::z[i*channels];
		}
