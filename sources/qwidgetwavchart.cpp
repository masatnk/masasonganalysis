...
void QWidgetWAVChart::initParams(){
    ...
    chartLayout->addWidget(chartview, 1, 0);
...
void QWidgetWAVChart::calcSpectrogram(){
    ...
    double max_freq = 1*44100;
    double min_freq = 1/winwid;
...
void QWidgetWAVChart::paintEvent(QPaintEvent* event){
    QVector<QVector<double>>& Spec_mag = parentwindow->getSpec_mag(); // If this is reference, crash when clearing Spec_mag in mainwindow.cpp
    if (Spec_mag.length() == 0 || specnumx == 0 || specnumy == 0) {
        return;
