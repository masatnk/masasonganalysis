...
void MainWindow::pushFilter(){
    ...
    wave0 = filterWave(anaDataL[0], fp);
    addAnaWave(wave0);
}

void MainWindow::addAnaWave(QVector<double> wave0){
    /*
    anaDataL.resize(anaDataL.length()+1);
    int anaind = anaDataL.length()-1;
    anaDataL[anaind].resize(wavDataL.length());
    anaDataL[anaind] = wave0;
    */
    anaDataL.append(wave0);
    int anaind = anaDataL.length()-1;
...
void MainWindow::pushWaves(){
    ...
    seriesYaxes.resize(anaDataL.length());
    for (int kk=0; kk<anaDataL.length(); kk++){
...
void MainWindow::pushRhythm(){
    ...
    fp.LPattn = 50;
    fp.LPripple = 0.1;
    anaDataL[anaind] = filterWave(anaDataL[0], fp);

    // Rectification
    anaDataL[anaind] = absWave(anaDataL[anaind]);

    // Discretize with moving average of step 10pnt (10/44100 s) 0.01 * 44100 = 441
    double stept = 0.01; // [s]
    int steppnt = int(stept * SamplesPerSec);
    anaDataL[anaind] = avgDiscretizeWave(anaDataL[anaind], steppnt);
    QVector<double> wavtdiscretize;
    wavtdiscretize = avgDiscretizeWave(wavt, steppnt);

    // log-transform wave
    for (int ii=0; ii<anaDataL.length(); ++ii){
        anaDataL[anaind][ii] = log(anaDataL[anaind][ii])/log(10)*10;
    }

    // differentiate wave
    for (int ii=0; ii<anaDataL[anaind].length()-1; ++ii){
        anaDataL[anaind][ii] = anaDataL[anaind][ii+1]-anaDataL[anaind][ii];
        wavtdiscretize[ii] = (wavtdiscretize[ii+1]+wavtdiscretize[ii])/2;
    }
    anaDataL[anaind].resize(anaDataL[anaind].length()-1);
    wavtdiscretize.resize(wavtdiscretize.length()-1);

    // LP filter < 20Hz
    struct filtparam fp2;
    fp2.samplerate = round(double(SamplesPerSec)/double(steppnt));
    fp2.HPfreq = 0;
    fp2.HPattn = 50;
    fp2.HPripple = 0.1;
    fp2.LPfreq = 20;
    fp2.LPattn = 50;
    fp2.LPripple = 0.1;
    anaDataL[anaind] = filterWave(anaDataL[anaind], fp2);

    specs[0].freq = round(double(SamplesPerSec)/double(steppnt));
    specs[0].winwid = 1;
    specs[0].padwid = 1;
    specs[0].shiftwid = 0.02;
    calcSpectrogram(anaDataL[anaind], wavtdiscretize, specs[0]); //t=0.012483 (0) - 15886.5 (1588648)
    widgetchart0->setAxisYspec(0, 0, 50);
    widgetchart0->prepareSpecImage();
    QMessageBox::information(this, "file path", "done");
}
...
void MainWindow::detectSound(){
    ...
    double winstepsize = 1.0 / SamplesPerSec;
    double sampling = SamplesPerSec;
    ...
    for(int ii=0; ii<lenwav-pntwinsize+1; ii+=winstep){
        ...
        for(int jj=0; jj<pntwinsize; jj++){ // this algorithm is numerically instable. use Welford's algorithm instead
            ...
            tmpavg = avg;
            avg += anaDataL[1][ii+jj];
            sd += anaDataL[1][ii+jj] * anaDataL[1][ii+jj];
        ...
        if (sd > avgbase + thres * sdbase){
            if (flagbase){
                flagbase = false;
                ton.append(wavt[ii+int(pntwinsize/2)]);
            }
        } else {
            if (!flagbase){
                flagbase = true;
                toff.append(wavt[ii+int(pntwinsize/2)]);
            }
            avgbase = avgbase*nbase/(nbase+1) + sd/(nbase+1);
            varbase = varbase*nbase/(nbase+1) + sd*sd/(nbase+1);
            sdbase = sqrt(varbase - avgbase*avgbase);
...
void MainWindow::openDir(QString pathname){
    ...
    if (!pathname.isEmpty()){
        ...
        QUrl fileurl = QFileDialog::getExistingDirectoryUrl(this, "Open dir", QUrl(filePath), QFileDialog::ShowDirsOnly);
    ...
    this->setWindowTitle("masaWavRead: " + filePath);
    appbusy = false;
...
// This might be better: "https://www.ipentec.com/document/csharp-play-wave-file-using-wave-api"
    if (appbusy || filePath.isEmpty()){
        return;
    }

    QString fileNametmp = listwidget->currentItem()->text();
    QString filePathtmp = filePath + "/" + fileNametmp;
    ...
    if (fileNametmp.isEmpty()){
        ...
        in >> Subchunk2Size;
        datasamplesize = Subchunk2Size / (NCh * bitsPerSample/8);
        if (NCh == 1){
            wavDataL.resize(datasamplesize);
            for(int ii=0; ii<wavDataL.size(); ii++){
                in >> wavDataL[ii];
            }
        } else if (NCh == 2){
            wavDataL.resize(datasamplesize);
            wavDataR.resize(datasamplesize);
            for(int ii=0; ii<wavDataL.size(); ii++){
                in >> wavDataL[ii];
                in >> wavDataR[ii];
            }
    ...
    if (wavDataL.length() > 0){
        anaDataL.resize(1);
        anaDataL[0].resize(wavDataL.size());
        for(int ii=0; ii<wavDataL.size(); ii++){
            anaDataL[0][ii] = double(wavDataL[ii]);
        }
    }
    if (wavDataR.length() > 0){
        anaDataR.resize(1);
        anaDataR[0].resize(wavDataL.size());
        for(int ii=0; ii<wavDataR.size(); ii++){
            anaDataR[0][ii] = double(wavDataR[ii]);
        }
    }
    init();
    ...
    specs[0].freq = SamplesPerSec;
    specs[0].winwid = 0.01;
    specs[0].padwid = 0.01;
    specs[0].shiftwid = 0.005;
    widgetchart0->setAxisYspec(0, 0, 20000);
    calcSpectrogram(anaDataL[0], wavt, specs[0]);
...
void MainWindow::calcSpectrogram(QVector<double> wavey, QVector<double> wavex, struct spec f){
    ...
    Spec_dx = 0;
    Spec_dy = 0;
    //QMessageBox::information(this, "!Done", "now");

    /*
    for(int ii=0; ii<f.mag.size(); ii++){
        QMessageBox::information(this, "!Done", QString::number(ii) + ":" + QString::number(f.mag[ii][0]) + "," + QString::number(f.phase[ii][1]));
        f.mag[ii].clear();
        f.phase[ii].clear();
    }
    */
    ...
    int freq = f.freq;
    double winwid = f.winwid;
    double padwid = f.padwid;
    double shiftwid = f.shiftwid;
    ...
    int enpnt = int((ent-stt)*f.freq-1); // end point of calculation
    ...
    int winpntorg = int(f.winwid*f.freq); // f.winwid*44100
    int padpnt = int(f.padwid*f.freq);
    int shiftpnt = int(f.shiftwid*f.freq); // this truncation will change the calc times

    double max_freq = 1*f.freq;
    ...
    do {
        ...
        for(int ii=0; ii<winpnt; ii++){
            if (ii < (padpnt-winpnt)/2 || (padpnt+winpnt)/2 <= ii){
                ...
                in[ii][0] = wavey[stwin+ii] * hanning; // real value
            }
        ...
        Spec_x[sindex] = stt + f.winwid/2 + double(sindex * shiftpnt) / f.freq;
        //f.x[sindex] = stt + f.winwid/2 + double(sindex * shiftpnt) / f.freq;
...
void MainWindow::calcSnipWAV(){
    ...
    int stpnt = int(stt * SamplesPerSec);
    int enpnt = int(ent * SamplesPerSec);
    if (stpnt > enpnt){
    ...
    if (enpnt >= wavDataL.length()){
        enpnt = wavDataL.length() - 1;
    }
    int lenpnt = enpnt - stpnt + 1;  // including enpnt
    wavSnipL.resize(lenpnt);
    if (NCh == 1){
        for(int ii=0; ii<lenpnt; ii++){
            wavSnipL[ii] = wavDataL[ii+stpnt]; // 2
        }
    } else if (NCh == 2){
        wavSnipR.resize(lenpnt);
        for(int ii=0; ii<lenpnt; ii++){
            wavSnipL[ii] = wavDataL[ii+stpnt]; // 2
            wavSnipR[ii] = wavDataR[ii+stpnt]; // 2
        }
    }
}
...
void MainWindow::pushPlaySoundPart(){
    ...
    if (lenpnt <= 0) return;
    qint32 chunksize = lenpnt * (NCh * bitsPerSample/8) - 46; // *** This determines the duration of playback. I don't know why but -46 (header size) works the best so far, although this is less than the actual data size.
...
void MainWindow::pushSaveWAV(){
    ...
    int lenpnt = wavSnipL.length();
    if (lenpnt <= 0) return;
    qint32 chunksize = lenpnt * (NCh * bitsPerSample/8); // *** Although I add -46 for playnback, I don't do that here as this is the actual data size.
